...
export default function ProductReportPage() {
  // ── Core state ──────────────────────────────────────────────────────────
  const [products, setProducts] = useState([]); // visible/revealed slice
  ...
  const loadNextChunk = useCallback(() => {
    ...
    const nextSlice = allProductsRef.current.slice(start, end);
  ...
  const handleLoadMore = () => {
    ...
    if (cursorRef.current >= allProductsRef.current.length) return;
    setLoadingMore(true);
    setTimeout(() => {
      loadNextChunk();
  ...
  useEffect(() => {
    if (startDate && endDate) fetchProducts();
  ...
  const totals = useMemo(() => {
    ...
    return {
      count: all.length,
      quantity: all.reduce((s, r) => s + toNum(r.quantity), 0),
      gstTotal: all.reduce((s, r) => s + toNum(r.gstAmount), 0),
      lineTotal: all.reduce((s, r) => s + toNum(r.lineTotal), 0),
  ...
  const buildHTML = () => {
    ...
    const totalRow = `
      <tr style="font-weight:700; border-top:2px solid #333;">
        ...
        <td style="padding:6px; font-size:11px; border:1px solid #ddd;"></td>
        <td style="padding:6px; font-size:11px; border:1px solid #ddd;"></td>
        <td style="padding:6px; font-size:11px; border:1px solid #ddd; text-align:right;">${totals.quantity}</td>
        <td style="padding:6px; font-size:11px; border:1px solid #ddd;"></td>
        <td style="padding:6px; font-size:11px; border:1px solid #ddd; text-align:right;">${currency(totals.gstTotal)}</td>
        <td style="padding:6px; font-size:11px; border:1px solid #ddd; text-align:right;">${currency(totals.lineTotal)}</td>
        <td style="padding:6px; font-size:11px; border:1px solid #ddd; text-align:right;">${currency(totals.grandTotal)}</td>
      </tr>`;
    ...
    return `
      ...
      <body>
        ...
        <table>
          <thead>${headRow}</thead>
          <tbody>${bodyRows || `<tr><td colspan="11" align="center">No records</td></tr>`}${totalRow}</tbody>
        </table>
  ...
  const exportExcel = () => {
    ...
    try {
      const data = allProductsRef.current.map((p) => ({
      ...
      a.href = url;
      a.download = `Product_Sale_Report_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  const hasMore = cursorRef.current < allProductsRef.current.length;
      ...
      <div className="px-4 md:px-6 pt-4 pb-3 shrink-0">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-bold text-slate-900">Product Report</h1>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={exportPDF} disabled={!products.length || loading} className="h-9 px-3 rounded-lg border border-slate-200 text-red-600 hover:bg-red-50 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1.5 text-sm font-medium">
              <FileText className="w-4 h-4" />PDF
            </button>
            <button onClick={exportExcel} disabled={!products.length || loading} className="h-9 px-3 rounded-lg border border-slate-200 text-emerald-600 hover:bg-emerald-50 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1.5 text-sm font-medium">
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-3 flex items-center gap-2 flex-wrap">
          ...
          <button onClick={fetchProducts} disabled={!startDate || !endDate || loading}
          ...
          <>
            ...
            <div className="bg-white border border-slate-200/80 rounded-2xl px-5 py-3.5 mb-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <SummaryStat label="Records" value={String(totals.count)} />
                <SummaryStat label="Quantity" value={String(totals.quantity)} />
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 text-slate-500 text-xs uppercase tracking-wide">
                      ...
                      <th className="text-right font-semibold px-4 py-3">Grand Total</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {products.map((p, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-right text-slate-600">{currency(p.price)}</td>
                        <td className="px-4 py-3 text-right text-slate-500">{p.gstRate ?? 0}%</td>
                        <td className="px-4 py-3 text-right text-slate-600">{currency(p.lineTotal)}</td>
                        <td className="px-4 py-3 text-right font-semibold text-slate-800">
                          {currency(p.grandTotal)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
